...
const GenerateProgramPage = () => {
  ...
  //  SIDE EFFECT FOR MEETING END ERROR
  useEffect(() => {
  ...
  // REDIRECT USER TO PROFILE PAGE AFTER CALL ENDED
  useEffect(() => {
    if (callEnded) {
  ...
  const toggleCall = async () => {
    if (callActive) vapi.stop();
      ...
      try {
        ...
        // const workflowId =
        //   language === 'ua'
        //     ? process.env.NEXT_PUBLIC_VAPI_WORKFLOW_ID_UK
        ...
        //   }
        // });
        await vapi.start(process.env.NEXT_PUBLIC_VAPI_WORKFLOW_EN_ID!, {
          variableValues: {
            first_name: user?.firstName,
            user_id: user?.id
          }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          ...
          <Card className="bg-card/90 backdrop-blur-sm border border-border overflow-hidden relative">
            <div className=" flex flex-col items-center justify-center p-6 relative -webkit-box-pack: center">
              ...
              <h2 className="text-xl font-bold text-foreground">GenGym AI</h2>
              ...
              {messages.map((msg, index) => (
                <div key={index} className="message-item animate-fadeIn">
                  <div className="font-semibold text-xs text-muted-foreground mb-1">
                    {msg.role === 'assistant' ? 'GenGym AI' : 'You'}:
                  </div>
